        ...
        <div class="layout">
            <div  class="layout-topbar clearfix">
                <div class="layout-name" :class="{'sider-mini': isCollapse}">
                    <router-link class="full" to="/" v-if="!isCollapse">{{$store.getters.APP_NAME}}</router-link>
                    <span class="mini" v-else>P</span>
                </div>
                <div class="layout-collapse" @click="toggleSider"><i class="fa fa-bars"></i></div>
                <div class="layout-nav">
                    <div v-loading.fullscreen.lock="$store.getters.LOADING"  @click="handleLogout"> <i class="fa fa-sign-out" aria-hidden="true"></i></div>
                </div>
...
<script>
    ...
    export default {
        ...
        mounted () {
            var vm = this
            vm.$store.dispatch('MOUNTED')
            vm.currentRoute = this.$route.name
        },
        watch: {
            $route (newVal, oldVal) {
